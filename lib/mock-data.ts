...
export function getProblems(lang:Lang):Problem[]{
  return problemSeed.map(p=>({
    id:p.id,
    icon:p.icon,
    title:pick(p.title,lang),
    hint:pick(p.hint,lang),
  }))
}
...
export function getMasters(lang:Lang):Master[]{
  return masterSeed.map(m=>({
    name:m.name,
    role:pick(m.role,lang),
    rating:m.rating,
    reviews:m.reviews,
    eta:lang==='en'?`${m.etaMin} min`:lang==='kk'?`${m.etaMin} мин`:`${m.etaMin} мин`,
    distance:`${m.distanceKm} км`,
    price:lang==='en'
      ? `from ${m.price.toLocaleString('en-US')} ₸`
      : lang==='kk'
        ? `${m.price.toLocaleString('ru-RU')} ₸ бастап`
        : `от ${m.price.toLocaleString('ru-RU')} ₸`,
    initials:m.initials,
    phone:m.phone,
    coords:m.coords,
  }))
...
export function getStations(lang:Lang){
  return stationSeed.map(s=>({
    ...s,
    type:pick(s.type,lang),
    open:pick(s.open,lang),
  }))
}
...
export const statusText:Record<OrderStatus,L>={
  ...
  'Ищем мастера':{
    kk:'Шебер ізделуде',
    ru:'Ищем мастера',
    en:'Searching for a specialist',
  },
  'Мастер принял заказ':{
    kk:'Шебер тапсырысты қабылдады',
    ru:'Мастер принял заказ',
    en:'Specialist accepted the job',
  },
  'Мастер едет':{
    kk:'Шебер жолда',
    ru:'Мастер едет',
    en:'Specialist is on the way',
  },
  'Мастер прибыл':{
    kk:'Шебер келді',
    ru:'Мастер прибыл',
    en:'Specialist has arrived',
  },
  'Работа выполняется':{
    kk:'Жұмыс орындалуда',
  ...
  'Нет свободных специалистов':{
    kk:'Бос мамандар жоқ',
    ru:'Нет свободных специалистов',
    en:'No specialists available',
  },
  'Завершён':{
    kk:'Аяқталды',
    ru:'Завершён',
